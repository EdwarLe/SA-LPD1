...
export const compareNumbers = (n1, n2, n3) => {
    ...
    for(let i = 0; i < numberArr.length; i++) {
        ...
        for(let j = 0; j < numberArr.length; j++) {
            if(numberArr[i] === numberArr[j] && i !== j) {
                valorRepetido = `El número ${numberArr[i]} está repetido`
    ...
    const sortHig = [numeroBajo, numeroMedio, numeroAlto]
    const sortlow = [numeroAlto, numeroMedio, numeroBajo]
